...
- **Admin page: Saved Teams panel.** Lists every team saved from the Status Report page with its members, and lets **department leads, admins, or the ckonkol account** remove teams (staged removals commit via a **Save Changes** button). For anyone else the Remove buttons and Save Changes stay disabled with a note explaining why. Removing a team only deletes the saved grouping — no user accounts or briefing data are touched.
...
- **Username and Logout now live only in the shared navigation bar** (`nav.js`): when signed in, the bar shows the account and a Logout link on every page. The User Management header's own email/Logout were removed — the nav bar is the single sign-out spot site-wide.
- Nav bar's **Briefing / Status Report links only use the remembered user ID while signed in.** Signed-out visitors clicking them get the plain pages (enter-ID / connect screens) instead of the last saved user's report. An ID in the current page's URL still carries across the two links either way.
- Nav bar renders at the **same 1100px width on every page** (matching the Admin page). It previously shrank inside the width-constrained Briefing (900px) and Help (800px) page bodies; it now centers itself on the viewport instead of its parent.
- Nav bar shows the **user ID only** (email with the @domain stripped — "ckonkol", not "ckonkol@aqua-aerobic.com"); the full email remains as a hover tooltip. The Help page now loads Firebase Auth too, so the signed-in user ID + Logout appear on **all five nav areas**: Briefing, Dashboard, Status Report, Help, and Admin.
- **Status Report: viewer vs manager toolbars.** For everyone who is not a signed-in lead/admin (or ckonkol), the **Add user…** dropdown/input and **💾 Save team** button are now hidden entirely — regular users viewing their report (e.g. `?report=mhevey`) see only the viewing tools. In their place, everyone gets **💾 Save report**: it stores the current section toggles, sort order, and Projects-panel show/hide/order on the user's own briefing document (`reportView` field), and the report re-applies that saved view automatically whenever it's opened directly (any device, any browser). Team-opened reports still use the team's saved settings, and **Save report is hidden on team reports** (e.g. `?team=IT Steering`) — saving there would overwrite every member's personal view; teams are saved with Save team instead.
